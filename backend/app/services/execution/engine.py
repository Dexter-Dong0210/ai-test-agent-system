...
# type: ignore  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2ZGxONWRBPT06MTU0OTViM2M=
        ...
        try:
            ...
            async with async_session_factory() as session:
                run_repo = TestRunRepository(session)
                test_run = await run_repo.get_by_id(test_run_id)
# pragma: no cover  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2ZGxONWRBPT06MTU0OTViM2M=
...
# fmt: off  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2ZGxONWRBPT06MTU0OTViM2M=
    ...
    async def _run_job(self, test_run_id: UUID, job: TestRunScriptJob) -> ExecutionResult:
        ...
        # 实时更新测试运行进度（显示 in_progress）
        await self._update_test_run_progress(test_run_id)
...
# noqa  My80OmFIVnBZMlhva2FQbHNJL21tS1U2ZGxONWRBPT06MTU0OTViM2M=
    ...
    async def _update_test_run_progress(self, test_run_id: UUID) -> None:
        """实时更新测试运行进度统计"""
        try:
            async with async_session_factory() as session:
                run_repo = TestRunRepository(session)
            ...
            logger.warning("[ScriptExecutionEngine] 更新测试运行进度失败: %s", e)
