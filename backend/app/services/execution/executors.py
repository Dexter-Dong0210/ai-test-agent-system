...
# fmt: off  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2VjB4clJRPT06NjJmNmYwMjQ=
...
# fmt: off  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2VjB4clJRPT06NjJmNmYwMjQ=
...
# pylint: disable  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2VjB4clJRPT06NjJmNmYwMjQ=
                ...
                # 生成执行日志
                stdout_lines = []
                stdout_lines.append(f"场景测试执行日志: {scenario_name}")
                stdout_lines.append(f"运行标识: {scenario_run.identifier}")
                stdout_lines.append(f"状态: {scenario_run.status}")
                stdout_lines.append(f"总步骤数: {result_summary['total']}")
                stdout_lines.append(f"通过: {result_summary['passed']}")
                stdout_lines.append(f"失败: {result_summary['failed']}")
                stdout_lines.append(f"跳过: {result_summary['skipped']}")
                stdout_lines.append(f"执行时间: {duration_ms}ms")
                stdout_lines.append("\n步骤执行详情:")
                stdout_lines.append("=" * 80)
                
                for sr_data in step_results_data:
                    stdout_lines.append(f"\n步骤 {sr_data['step_order']}: {sr_data['step_name']}")
                    stdout_lines.append(f"  状态: {sr_data['status']}")
                    if sr_data.get('duration_ms'):
                        stdout_lines.append(f"  耗时: {sr_data['duration_ms']}ms")
                    if sr_data.get('error_message'):
                        stdout_lines.append(f"  错误: {sr_data['error_message']}")
                    if sr_data.get('request_data'):
                        stdout_lines.append(f"  请求: {sr_data['request_data']}")
                    if sr_data.get('response_data'):
                        stdout_lines.append(f"  响应: {sr_data['response_data']}")
                    if sr_data.get('assertion_results'):
                        stdout_lines.append(f"  断言结果: {sr_data['assertion_results']}")
                
                stdout_content = "\n".join(stdout_lines)
                stderr_content = scenario_run.error_message if not success else None
                ...
                return ExecutionResult(
                    ...
                    stdout=stdout_content[:50000] if stdout_content else "",
                    stderr=stderr_content[:50000] if stderr_content else "",
                )
...
# type: ignore  My80OmFIVnBZMlhva2FQbHNJL21tS1U2VjB4clJRPT06NjJmNmYwMjQ=
