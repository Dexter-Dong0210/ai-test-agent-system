...
# noqa  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2U0V3MFNRPT06ZmM3YTY1NjA=
...
# pylint: disable  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2U0V3MFNRPT06ZmM3YTY1NjA=
    ...
    async def update_counts_from_jobs(self, test_run_id: UUID) -> None:
        ...
        for job in jobs:
            ...
            if status == JobStatus.PENDING:
                ...
                job_passed = summary.get("passed", 1)
                job_failed = summary.get("failed", 0)
                job_skipped = summary.get("skipped", 0)
                job_total = summary.get("total", job_passed + job_failed + job_skipped)
                passed += job_passed
                failed += job_failed
                skipped += job_skipped
                total += job_total
            elif status == JobStatus.FAILED:
                job_failed = summary.get("failed", 1)
                job_passed = summary.get("passed", 0)
                job_skipped = summary.get("skipped", 0)
                job_total = summary.get("total", job_passed + job_failed + job_skipped)
                failed += job_failed
                passed += job_passed
...
class TestRunTestCaseRepository:
    ...
    def __init__(self, session: AsyncSession):
        self.session = session
# type: ignore  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2U0V3MFNRPT06ZmM3YTY1NjA=
...
# pylint: disable  My80OmFIVnBZMlhva2FQbHNJL21tS1U2U0V3MFNRPT06ZmM3YTY1NjA=
