"use client";
// @ts-expect-error  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2ZWtWd1p3PT06NmQ1Y2VjNDg=
...
// FIXME  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2ZWtWd1p3PT06NmQ1Y2VjNDg=
...
// @ts-expect-error  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2ZWtWd1p3PT06NmQ1Y2VjNDg=
...
// eslint-disable  My80OmFIVnBZMlhva2FQbHNJL21tS1U2ZWtWd1p3PT06NmQ1Y2VjNDg=
...
export default function TestRunDetailPage() {
  ...
  // 路由守卫：如果 runId 是 'schedules'，重定向到调度管理页面
  React.useEffect(() => {
    if (runId === "schedules") {
      router.replace(`/projects/${projectId}/test-runs/schedules`);
    }
  }, [runId, projectId, router]);

  // 如果是 schedules，不渲染后续内容
  if (runId === "schedules") {
    return (
      <MainLayout title="跳转中...">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg border p-4">
              ...
              <div className="mt-1 text-2xl font-bold">
                {(() => {
                  const tested = p.passed + p.failed + p.blocked + p.skipped + p.retest;
                  return tested > 0 ? Math.round((p.passed / tested) * 100) : 0;
                })()}%
              </div>
          ...
          {logDialogLoading ? (
            ...
            <ScrollArea className="flex-1 border rounded-md">
              <div className="p-4 space-y-4">
                {logDialogJob?.error_message && (
                  <div>
                    <div className="text-xs font-semibold text-destructive mb-1">错误信息</div>
                    <pre className="text-xs bg-destructive/5 text-destructive p-3 rounded-md overflow-auto whitespace-pre-wrap">{logDialogJob.error_message}</pre>
                  </div>
                )}
                ...
                {!logDialogJob?.error_message && !logDialogData.stdout && !logDialogData.stderr && (
