 ...
 */
// FIXME  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0VOSFl3PT06Zjg4Y2RjOGM=
...
import { McpToolManager } from '../core/llm/mcp-client';
...
// eslint-disable  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0VOSFl3PT06Zjg4Y2RjOGM=
...
// eslint-disable  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0VOSFl3PT06Zjg4Y2RjOGM=
...
// @ts-expect-error  My80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0VOSFl3PT06Zjg4Y2RjOGM=
...
const AppStateProviderInner = ({ children }: { children: ReactNode }) => {
  ...
  const mcpManagerRef = useRef<McpToolManager | null>(null);
  ...
  const initializeAgent = useCallback(
    async (overrideProjectName?: string): Promise<void> => {
      ...
      try {
        ...
        // Disconnect previous MCP connections before creating new ones
        if (mcpManagerRef.current) {
          await mcpManagerRef.current.disconnectAll();
          mcpManagerRef.current = null;
        }

        // Load MCP servers from settings and fetch their tools
        ...
        if (enabledServers.length > 0) {
          try {
            const { manager, result } = await McpToolManager.createWithServers(enabledServers);
            mcpManagerRef.current = manager;
            mcpTools = result.tools;
            const mcpStatusErrors = [...result.errors];
            if (result.tools.length === 0 && result.errors.length === 0) {
              mcpStatusErrors.push('MCP servers connected but returned 0 tools. Check that the server has tools registered.');
            }
            setMcpErrors(mcpStatusErrors);
            if (import.meta.env.DEV) {
              console.log(
                `🔌 MCP: connected ${enabledServers.length} servers, loaded ${result.tools.length} tools`,
                result.tools.map((t: any) => t.name),
              );
              if (result.errors.length > 0) {
                console.warn('MCP connection errors:', result.errors);
              }
            }
          } catch (mcpError) {
            const errMsg = mcpError instanceof Error ? mcpError.message : String(mcpError);
            setMcpErrors([`MCP initialization failed: ${errMsg}`]);
            if (import.meta.env.DEV) {
              console.warn('Failed to initialize MCP tools:', mcpError);
            }
          }
        } else {
          setMcpErrors([]);
          if (import.meta.env.DEV) {
            console.log('🔌 MCP: no enabled servers');
          }
        }

        agentRef.current = createGraphRAGAgent(config, backend, codebaseContext, mcpTools);
        setIsAgentReady(true);
        setAgentError(null);
        if (import.meta.env.DEV) {
          const totalTools = (mcpTools?.length || 0) + 7;
          console.log(`✅ Agent initialized with ${totalTools} tools (${mcpTools?.length || 0} MCP)`);
        }
      } catch (error) {
