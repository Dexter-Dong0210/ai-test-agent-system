 ...
 */
// TODO  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2Y1RSVU1RPT06MDUwMDY0MWY=
...
// Dim a color by mixing with background (keeps color hint)
const dimColor = (hex: string, amount: number, isDark: boolean = true): string => {
  ...
  if (isDark) {
    ...
    const lightBg = { r: 248, g: 249, b: 250 }; // #f8f9fa - light background
    return rgbToHex(
      lightBg.r + (rgb.r - lightBg.r) * amount,
      lightBg.g + (rgb.g - lightBg.g) * amount,
      lightBg.b + (rgb.b - lightBg.b) * amount,
    );
  }
};
// FIXME  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2Y1RSVU1RPT06MDUwMDY0MWY=
...
// @ts-expect-error  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2Y1RSVU1RPT06MDUwMDY0MWY=
...
// TODO  My80OmFIVnBZMlhva2FQbHNJL21tS1U2Y1RSVU1RPT06MDUwMDY0MWY=
...
export const useSigma = (options: UseSigmaOptions = {}): UseSigmaReturn => {
  ...
  useEffect(() => {
    ...
    const sigma = new Sigma(graph, containerRef.current, {
      ...
      defaultNodeColor: 'var(--color-text-muted)',
      defaultEdgeColor: 'var(--color-border-default)',
      ...
      defaultDrawNodeHover: (context, data, settings) => {
        ...
        // Background pill - 使用 CSS 变量
        context.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--color-surface').trim() || '#12121c';
        context.beginPath();
        ...
        // Label text - 使用 CSS 变量
        context.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--color-text-primary').trim() || '#f5f5f7';
