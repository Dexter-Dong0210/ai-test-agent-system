 ...
 */
// FIXME  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2YzBob2VRPT06NTEzZTU2ZDA=
 ...
 */
// @ts-expect-error  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2YzBob2VRPT06NTEzZTU2ZDA=
...
// @ts-expect-error  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2YzBob2VRPT06NTEzZTU2ZDA=
  ...
  if (totalCount === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-6 text-center">
        ...
        <h3 className="mb-2 text-base font-medium text-text-primary">未检测到执行流程</h3>
        <p className="max-w-xs text-sm text-text-secondary">
          执行流程是从入口点追踪的执行流。加载代码库以查看检测到的流程。
        </p>
      </div>
    );
  }
        ...
        <div className="px-4 py-3">
          ...
          >
            ...
            <div className="flex-1">
              <h4 className="text-sm font-medium text-text-primary group-hover:text-cyan-600 dark:group-hover:text-cyan-200">
                完整流程图
              </h4>
              <p className="text-xs text-text-muted">查看 {totalCount} 个流程的合并图谱</p>
            </div>
            {loadingProcess === 'all' ? (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 text-xs text-text-muted">
          <span>{process.stepCount} 步骤</span>
          {process.clusters.length > 0 && (
            <>
              <span>•</span>
              <span>{process.clusters.length} 个集群</span>
            </>
          )}
        </div>
      </div>
      ...
      <button
        ...
        className={`rounded-md p-1.5 transition-all ${
          isFocused
            ? 'animate-pulse border border-amber-500/40 bg-amber-500/20 text-amber-600 dark:text-amber-400 opacity-100 hover:bg-amber-500/30 hover:text-amber-700 dark:hover:text-amber-300'
            : 'border border-black/10 bg-black/5 text-text-muted opacity-0 group-hover:opacity-100 hover:border-cyan-500/40 hover:bg-cyan-500/20 hover:text-cyan-600 dark:hover:text-cyan-400'
        }`}
        title={isFocused ? '点击取消图谱高亮' : '点击在图谱中高亮'}
      ...
      <button
        ...
        className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium shadow-sm transition-all disabled:opacity-50 ${
          isSelected
            ? 'border border-cyan-500/60 bg-cyan-100/60 dark:bg-cyan-900/60 text-cyan-700 dark:text-cyan-300 opacity-100'
            : 'border border-cyan-500/30 bg-cyan-50/30 dark:bg-cyan-950/30 text-cyan-600 dark:text-cyan-400 opacity-0 shadow-cyan-900/20 group-hover:opacity-100 hover:border-cyan-500/50 hover:bg-cyan-100/50 dark:hover:bg-cyan-900/50 hover:text-cyan-700 dark:hover:text-cyan-300'
        }`}
      >
        {isLoading ? (
...
// FIXME  My80OmFIVnBZMlhva2FQbHNJL21tS1U2YzBob2VRPT06NTEzZTU2ZDA=
