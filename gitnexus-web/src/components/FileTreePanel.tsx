...
// FIXME  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2TjJ0WlZ3PT06NmI5YTY5NmQ=
...
// TODO  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2TjJ0WlZ3PT06NmI5YTY5NmQ=
...
// eslint-disable  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2TjJ0WlZ3PT06NmI5YTY5NmQ=
...
const TreeItem = ({
  ...
  return (
    <div>
      <button
        onClick={handleClick}
        className={`relative flex w-full items-center gap-1.5 rounded px-2 py-1 text-left text-sm transition-colors hover:bg-hover ${isSelected ? 'border-l-2 border-amber-500 bg-amber-500/15 text-amber-700 dark:text-amber-300' : 'border-l-2 border-transparent text-text-secondary hover:text-text-primary'} ${matchesSearch ? 'bg-accent/10' : ''} `}
      ...
      {activeTab === 'files' && (
        <>
          ...
          <div className="scrollbar-thin flex-1 overflow-y-auto py-2">
            {fileTree.length === 0 ? (
              <div className="px-3 py-4 text-center text-xs text-text-muted">未加载文件</div>
            ) : (
              fileTree.map((node) => (
                <TreeItem
                  key={node.id}
        ...
        <div className="scrollbar-thin flex-1 overflow-y-auto p-3">
          ...
          <div className="mt-6 border-t border-border-subtle pt-4">
            ...
            {depthFilter !== null && !selectedNode && (
              <p className="mt-2 text-[10px] text-amber-400">选择节点以应用深度筛选</p>
            )}
          </div>
      ...
      {graph && (
        <div className="border-t border-border-subtle bg-elevated/50 px-3 py-2">
          <div className="flex items-center justify-between text-[10px] text-text-muted">
            ...
            <span>{graph.relationships.length} 边</span>
          </div>
        </div>
      )}
    </div>
...
// FIXME  My80OmFIVnBZMlhva2FQbHNJL21tS1U2TjJ0WlZ3PT06NmI5YTY5NmQ=
