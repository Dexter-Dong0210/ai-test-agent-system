 ...
 */
// eslint-disable  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0ZKclVBPT06ZmEzYzlmMGM=
...
// FIXME  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0ZKclVBPT06ZmEzYzlmMGM=
...
// @ts-expect-error  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0ZKclVBPT06ZmEzYzlmMGM=
...
export const ProcessFlowModal = ({
      ...
      try {
        ...
        diagramRef.current!.innerHTML = `
          <div class="text-center p-8">
            <div class="text-red-600 dark:text-red-400 text-sm font-medium mb-2">
              ${isSizeError ? '📊 图表太大' : '⚠️ 渲染错误'}
            </div>
            <div class="text-slate-400 text-xs max-w-md">
              ${
                isSizeError
                  ? `此图表有 ${process.steps?.length || 0} 个步骤，过于复杂无法渲染。尝试查看单个流程而不是"所有流程"。`
                  : `无法渲染图表。步骤数：${process.steps?.length || 0}`
              }
            </div>
          </div>
        `;
  ...
  return (
    ...
    >
      {/* Glassmorphism Modal - 支持主题切换 */}
      <div
        className={`animate-scale-in relative flex flex-col overflow-hidden rounded-3xl border border-border-subtle bg-surface shadow-2xl shadow-cyan-500/10 backdrop-blur-2xl ${
      ...
      >
        ...
        {/* Header */}
        <div className="relative z-10 border-b border-border-subtle px-6 py-5">
          <h2 className="text-lg font-semibold text-text-primary">流程：{process.label}</h2>
        </div>
        ...
        {/* Footer Actions */}
        <div className="relative z-10 flex items-center justify-center gap-3 border-t border-border-subtle bg-elevated/50 px-6 py-4">
          {/* Zoom controls */}
          <div className="flex items-center gap-1 rounded-lg border border-border-subtle bg-elevated p-1">
            <button
              onClick={handleZoomOut}
              className="rounded-md p-2 text-text-secondary transition-all hover:bg-hover hover:text-text-primary"
              title="缩小（-）"
            >
          ...
          <button
            onClick={resetView}
            className="flex items-center gap-2 rounded-lg border border-border-subtle bg-elevated px-4 py-2.5 text-sm font-medium text-text-secondary transition-all hover:bg-hover hover:text-text-primary"
            title="重置缩放和平移"
          >
            重置视图
          </button>
          {onFocusInGraph && (
            <button
              ...
              className="flex items-center gap-2 rounded-lg bg-node-function px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-node-function/20 transition-all hover:opacity-90"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-accent px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-accent/20 transition-all hover:bg-accent-dim"
          >
          ...
          <button
            onClick={onClose}
            className="rounded-lg border border-border-subtle bg-elevated px-5 py-2.5 text-sm font-medium text-text-secondary transition-all hover:bg-hover hover:text-text-primary"
          >
            关闭
          </button>
        </div>
...
// @ts-expect-error  My80OmFIVnBZMlhva2FQbHNJL21tS1U2Y0ZKclVBPT06ZmEzYzlmMGM=
