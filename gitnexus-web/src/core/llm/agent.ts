 ...
 */
// TODO  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2TjNwaVR3PT06ODQ0NjBiODM=
...
// NOTE  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2TjNwaVR3PT06ODQ0NjBiODM=
...
// TODO  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2TjNwaVR3PT06ODQ0NjBiODM=
...
export const createGraphRAGAgent = (
  ...
  const allTools = mcpTools ? [...tools, ...mcpTools] : tools;

  // Use dynamic prompt if context is provided, otherwise use base prompt
  let systemPrompt = codebaseContext
    ? buildDynamicSystemPrompt(BASE_SYSTEM_PROMPT, codebaseContext)
    : BASE_SYSTEM_PROMPT;

  // Append MCP tool references if provided
  if (mcpTools && mcpTools.length > 0) {
    const mcpToolDescriptions = mcpTools
      .map((t) => `- **\`${t.name}\`** — ${t.description || 'MCP tool'}`)
      .join('\n');
    systemPrompt += `\n\n## 🔌 EXTERNAL MCP TOOLS\nThe following external tools are also available via MCP servers:\n${mcpToolDescriptions}\n`;
  }
...
// NOTE  My80OmFIVnBZMlhva2FQbHNJL21tS1U2TjNwaVR3PT06ODQ0NjBiODM=
