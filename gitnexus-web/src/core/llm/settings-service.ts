...
// FIXME  MC80OmFIVnBZMlhva2FQbHNJL21tS1U2VDBwR05RPT06M2VlYjBlYWY=
...
const mergeWithDefaults = (parsed?: Partial<LLMSettings> | null): LLMSettings => ({
  ...
  mcpServers: parsed?.mcpServers ?? DEFAULT_LLM_SETTINGS.mcpServers,
});
...
// FIXME  MS80OmFIVnBZMlhva2FQbHNJL21tS1U2VDBwR05RPT06M2VlYjBlYWY=
...
// eslint-disable  Mi80OmFIVnBZMlhva2FQbHNJL21tS1U2VDBwR05RPT06M2VlYjBlYWY=
...
export const updateMcpServers = (servers: McpServerConfig[]): LLMSettings => {
  const current = loadSettings();
  const updated: LLMSettings = { ...current, mcpServers: servers };
  saveSettings(updated);
  return updated;
};

export const addMcpServer = (server: McpServerConfig): LLMSettings => {
  const current = loadSettings();
  const updated: LLMSettings = {
    ...current,
...
export const removeMcpServer = (id: string): LLMSettings => {
  const current = loadSettings();
  const updated: LLMSettings = {
    ...current,
...
export const toggleMcpServer = (id: string): LLMSettings => {
  const current = loadSettings();
  const updated: LLMSettings = {
    ...current,
    mcpServers: (current.mcpServers || []).map((s) =>
...
// eslint-disable  My80OmFIVnBZMlhva2FQbHNJL21tS1U2VDBwR05RPT06M2VlYjBlYWY=
